...
void SENNA_nn_lookup(float *dest, int dest_stride, const float *wordweights, int wordsize, //"*" represents pointers, which direct the memory of the 
int maxwordidx, const int *wordindices, int nword, int padidx, int npad)		   // previous variable to the one immediately follwing it. Func	
{										           // composition in F#?		
  int i; //word index 
								
  if(padidx < 0 || padidx >= maxwordidx) // padding index represents the space added to the word vectors in order to map them 1:1 to the 
    SENNA_error("lookup: padding index out of range"); // output vectors, based on the stride set by dest_stride. 

  for(i = 0; i < npad; i++) // sets the conditions for stride and padding that allow the vector for word weights to map directly/multiplicatively to word size   
    memcpy(dest+i*dest_stride, wordweights+padidx*wordsize, wordsize*sizeof(float)); // From this, features of the words can be assigned to various words by the backprop algorithm
										// cycles from index =0 all the way to npad by one
  for(i = 0; i < nword; i++) 
  {
    ...
    memcpy(dest+(i+npad)*dest_stride, wordweights+wordidx*wordsize, wordsize*sizeof(float)); // converts padding, stride, weight, index, and size to a vector
  }											     // that is copied to another location/vector
...
float *input, int input_frame_size, int n_frames, int k_w)
{
  int i, j, k;
  int h_k_w = (k_w-1)/2;
  ...
  for(k = 0; k < n_frames; k++)
  {
    ...
    {
      float maxval = -FLT_MAX;         // this function is done for all pairwise combinations 
				       // of k and i 
      for(j = -k; j < n_frames-k; j++) // that exist in the 0..n_frame and 0..input_frame_size set
      {                                // respectively
        int jbias = j + h_k_w;         // defining function variables for easier matching 
        int jinput = k+j;
        float z;
        ...
        if(z > maxval)
          maxval = z;              // maps z to each k,i coordinate
      }
      output[i+k*input_frame_size] = maxval; //maps output array to each k,i coordinate 
    }
  }
}
