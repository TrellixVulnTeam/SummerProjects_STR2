...
class Annotator:
    ...
    def getDependency(self, parse):
        ...
        parsefile.close()
        p = subprocess.Popen(
            ['java', '-cp', 'stanford-parser.jar', 'edu.stanford.nlp.trees.EnglishGrammaticalStructure', '-treeFile',
             cwd + '/in.parse', '-collapsed'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        p.wait()
...
def test():
    ...
    print annotator.getBatchAnnotations(
        ["He killed the man with a knife and murdered him with a dagger.", "He is a good boy."], dep_parse=True)
